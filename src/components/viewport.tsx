    ...
    const handleKeydown = (ev: KeyboardEvent) => {
      ...
      if (ev.key === "ArrowLeft") {
        ...
        const futureCurrentPieceInViewport = {
          ...currentPieceInViewport,
          atoms: rotateAtomsToTheRight(currentPieceInViewport.atoms),
        };
        const { width: pieceWidth } = getAtomsDimensions(futureCurrentPieceInViewport.atoms);

        const hasHitBorder = currentPieceInViewport.x + pieceWidth >= VIEWPORT_WIDTH;
        if (hasHitBorder) {
          const afa = futureCurrentPieceInViewport.x + pieceWidth - VIEWPORT_WIDTH;
